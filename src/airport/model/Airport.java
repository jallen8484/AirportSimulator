...
public class Airport extends AbstractModel {
	...
	*/
	...
	public void start(int listsize, double arrivalrate) {
		...
		while (it.hasNext()) {
			Airplane temp = it.next();
			if (temp.getTank().emergency()) queueAirportEvent.addFirst(new AirportEvent(temp,0, "New Emergency"));
			else queueAirportEvent.addLast(new AirportEvent(temp,0, "New Plane"));
		}
